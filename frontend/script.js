...
function addStorageInputRow() {
    ...
    tr.innerHTML = `
        ...
        <td><input type="text" class="input-full text-center storage-partition-input" placeholder="/os"></td>
        <td><input type="number" class="input-full text-center storage-used-input" value="0" min="0" step="0.01"></td>
        <td><input type="text" class="input-full storage-note-input" placeholder="VÃ­ dá»¥: DÃ¹ng cho app logs"></td>
        <td class="admin-cell">
        ...
        <td class="admin-cell">
            <input type="text" class="input-full admin-note storage-admin-note" placeholder="Nháº­n xÃ©t...">
        </td>
        <td class="text-center">
            <button class="btn-delete-row-item" onclick="${deleteRowHandler}">
                <i class="fa-solid fa-trash"></i>
...
function calculateSizingRecommendations() {
...

    {
        ...
        if (storageTotalsNew.length === 0) {
            showToast('Vui lÃ²ng nháº­p Ã­t nháº¥t má»™t phÃ¢n vÃ¹ng trong "THÃ”NG TIN LÆ¯U TRá»® Äáº¦U VÃ€O".', 'warning');
            return;
        }
        ...
        if (!virtualizationNew.selectedValue) {
            showToast('Vui lÃ²ng chá»n cáº¥u hÃ¬nh áº£o hÃ³a há»£p lá»‡ trÆ°á»›c khi tÃ­nh toÃ¡n.', 'warning');
            return;
        }
        ...
        const machineRowsNew = [
            {
                label: 'Cintrate cáº§n cho há»‡ thá»‘ng',
                value: cintForTPSNew.toFixed(2),
                note: `= ${totalCintNew.toFixed(2)} Ã— (${sizing} / ${poc}) = ${totalCintNew.toFixed(2)} Ã— ${factorNew.toFixed(4)}`
            },
            {
                label: 'RAM (GB) cáº§n cho há»‡ thá»‘ng',
                value: ramForTPSNew.toFixed(2),
                note: `= ${totalRamNew.toFixed(2)} Ã— (${sizing} / ${poc}) = ${totalRamNew.toFixed(2)} Ã— ${factorNew.toFixed(4)}`
            }
        ];

        storageAfterKPINew.forEach(item => {
            machineRowsNew.push({
                label: `${item.name} (GB) cáº§n cho há»‡ thá»‘ng`,
                value: item.forTPS.toFixed(2),
                note: `= ${item.totalUsed.toFixed(2)} Ã— (${sizing} / ${poc}) = ${item.totalUsed.toFixed(2)} Ã— ${factorNew.toFixed(4)}`
            });
        });

        machineRowsNew.push(
            {
                label: 'Cint cáº§n sau khi nhÃ¢n há»‡ sá»‘ dá»± phÃ²ng vÃ  Ä‘áº£m báº£o KPI',
                value: cintAfterKPINew.toFixed(2),
                note: `= ${cintForTPSNew.toFixed(2)} / 0.75 Ã— 1.1. KPI 75%, Sai sá»‘ 1.1`
            },
            {
                label: 'RAM cáº§n sau khi nhÃ¢n há»‡ sá»‘ dá»± phÃ²ng vÃ  Ä‘áº£m báº£o KPI',
                value: ramAfterKPINew.toFixed(2),
                note: `= ${ramForTPSNew.toFixed(2)} / 0.9 Ã— 1.1. KPI 90%, Sai sá»‘ 1.1`
            }
        );

        storageAfterKPINew.forEach(item => {
            machineRowsNew.push({
                label: `${item.name} cáº§n sau khi nhÃ¢n há»‡ sá»‘ dá»± phÃ²ng vÃ  Ä‘áº£m báº£o KPI`,
                value: item.afterKPI.toFixed(2),
                note: `= ${item.forTPS.toFixed(2)} / 0.8 Ã— 1.1. KPI 80%, Sai sá»‘ 1.1`
            });
        });
        ...
        const cintPerServerNew = virtualizationNew.mode === 'vcpu'
            ? virtualizationNew.vcpu
            : Math.ceil(cintAfterKPINew / ketquaNew);
        const ramPerServerNew = virtualizationNew.mode === 'ram'
            ? virtualizationNew.ram
            : Math.ceil(ramAfterKPINew / ketquaNew);
        ...
        let htmlNew = '';
        htmlNew += `<h4 style="margin-top:16px; margin-bottom:8px; color:#2c5282;">Báº£ng tÃ­nh toÃ¡n MÃ¡y chá»§ Tiáº¿n trÃ¬nh</h4>`;
        htmlNew += `<table class="sizing-table app-machine-table" data-app-machine-table="1" style="margin-top:8px;">
                        <thead>
                            <tr>
                                <th style="width:50px;">STT</th>
                                <th style="width:350px;">ThÃ´ng sá»‘</th>
                                <th style="width:150px;">MÃ¡y chá»§ Tiáº¿n trÃ¬nh</th>
                                <th>Ghi chÃº</th>
                            </tr>
                        </thead>
                        <tbody>`;
        ...
        htmlNew += `<div data-app-recommendation="1" style="margin-top:16px; padding:12px; background:#e6fffa; border-left:4px solid #38b2ac; border-radius:4px;">
                        <strong>Äá» xuáº¥t:</strong> Lá»±a chá»n cáº¥u hÃ¬nh áº£o hÃ³a ${recommendationTargetNew}, lá»±a chá»n sá»‘ N theo mode Ä‘Ã£ chá»n:
                        ${recommendationFormulaNew} â‰ˆ <strong>${ketquaNew}</strong>
                    </div>`;
        htmlNew += `<h4 style="margin-top:20px; margin-bottom:8px; color:#2c5282;">Báº£ng phÃ¢n bá»• theo sá»‘ lÆ°á»£ng N</h4>`;
        htmlNew += `<table class="sizing-table app-n-table" data-app-n-table="1" style="margin-top:8px;">
                        <thead>
                            <tr>
                                <th style="width:120px;">GiÃ¡ trá»‹ N</th>
                                <th>Cint CPU yÃªu cáº§u</th>
                                <th>RAM yÃªu cáº§u</th>
                                ${storageAfterKPINew.map(item => `<th>${escapeHtml(item.name)} yÃªu cáº§u</th>`).join('')}
                            </tr>
                        </thead>
                        <tbody>
                            <tr style="background:#f0f4f8;">
        ...
        htmlNew += `<h4 style="margin-top:20px; margin-bottom:8px; color:#2c5282;">Äá» xuáº¥t cáº¥u hÃ¬nh</h4>`;
        htmlNew += `<table class="sizing-table app-proposal-table" data-app-proposal-table="1" style="margin-top:8px;">
                        <thead>
                            <tr>
                                <th style="width:250px;">Cáº¥u hÃ¬nh</th>
                        ...
                        <tbody>
                            <tr style="background:#e6ffed;">
                                ...
                                <td><textarea class="input-full sizing-note" rows="1" style="resize:vertical;min-height:30px;">Dá»± phÃ²ng N+1</textarea></td>
                            </tr>
                        </tbody>
                    </table>`;
    ...
    const machineRows = [
        {
            ...
            note: `= ${totalCint.toFixed(2)} Ã— (${sizing} / ${poc}) = ${totalCint.toFixed(2)} Ã— ${factor.toFixed(4)}`
        },
        {
            label: 'RAM (GB) cáº§n cho há»‡ thá»‘ng',
            value: ramForTPS.toFixed(2),
            note: `= ${totalRam.toFixed(2)} Ã— (${sizing} / ${poc}) = ${totalRam.toFixed(2)} Ã— ${factor.toFixed(4)}`
        }
    ];
    storageAfterKPI.forEach(item => {
        machineRows.push({
            label: `${item.name} (GB) cáº§n cho há»‡ thá»‘ng`,
            value: item.forTPS.toFixed(2),
            note: `= ${item.totalUsed.toFixed(2)} Ã— (${sizing} / ${poc}) = ${item.totalUsed.toFixed(2)} Ã— ${factor.toFixed(4)}`
        });
    });
    machineRows.push(
        {
            ...
            note: `= ${cintForTPS.toFixed(2)} / 0.75 Ã— 1.1. KPI 75%, Sai sá»‘ 1.1`
        },
        {
            label: 'RAM cáº§n sau khi nhÃ¢n há»‡ sá»‘ dá»± phÃ²ng vÃ  Ä‘áº£m báº£o KPI',
            value: ramAfterKPI.toFixed(2),
            note: `= ${ramForTPS.toFixed(2)} / 0.9 Ã— 1.1. KPI 90%, Sai sá»‘ 1.1`
        }
    );
    storageAfterKPI.forEach(item => {
        machineRows.push({
