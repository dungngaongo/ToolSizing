...
public class ExportService {
    ...
    private static final int MAX_IMAGE_WIDTH_PX = 450;
    ...
    private void writeModuleCustom(XWPFDocument doc, JsonNode moduleCustom, String heading, ExportContext context) {
        ...
        if ("linearEquivalentApp".equals(selectedMethod)) {
            ...
            writeModuleApp(doc, linearData, heading , context);
            return;
        }
    ...
    private Dimension calculateScaledDimensions(int originalWidth, int originalHeight) {
        if (originalWidth <= 0 || originalHeight <= 0) {
            return new Dimension(MAX_IMAGE_WIDTH_PX, MAX_IMAGE_HEIGHT_PX);
        }

        double widthRatio = (double) MAX_IMAGE_WIDTH_PX / originalWidth;
        double heightRatio = (double) MAX_IMAGE_HEIGHT_PX / originalHeight;
        double scaleRatio = Math.min(widthRatio, heightRatio);

        if (scaleRatio < 1.0) {
            int scaledWidth = (int) Math.round(originalWidth * scaleRatio);
            int scaledHeight = (int) Math.round(originalHeight * scaleRatio);
        ...
        try {
            ...
            try (ByteArrayInputStream bis = new ByteArrayInputStream(imageBytes)) {
                ...
                if (originalImage != null) {
                    ...
                    Dimension scaledSize = calculateScaledDimensions(originalWidth, originalHeight);

                    try (ByteArrayInputStream bis2 = new ByteArrayInputStream(imageBytes)) {
                        r.addPicture(bis2, pictureType, "image",
                            Units.toEMU(scaledSize.width), Units.toEMU(scaledSize.height));
                    }
                } else {
                    log.warn("Failed to read image, using default dimensions");
                    try (ByteArrayInputStream bis3 = new ByteArrayInputStream(imageBytes)) {
                        r.addPicture(bis3, pictureType, "image",
                            Units.toEMU(MAX_IMAGE_WIDTH_PX), Units.toEMU(MAX_IMAGE_HEIGHT_PX));
                    }
                }
            }
        } catch (IOException e) {
            ...
            try {
                byte[] imageBytes = Base64.getDecoder().decode(
                    base64String.contains(",") ? base64String.split(",", 2)[1] : base64String);
                ...
                try (ByteArrayInputStream bis = new ByteArrayInputStream(imageBytes)) {
                    int pictureType = XWPFDocument.PICTURE_TYPE_PNG;
                    if (base64String.toLowerCase().contains("jpeg") || base64String.toLowerCase().contains("jpg")) {
                        pictureType = XWPFDocument.PICTURE_TYPE_JPEG;
                    } else if (base64String.toLowerCase().contains("gif")) {
                        pictureType = XWPFDocument.PICTURE_TYPE_GIF;
                    } else if (base64String.toLowerCase().contains("bmp")) {
                        pictureType = XWPFDocument.PICTURE_TYPE_BMP;
                    }

                    r.addPicture(bis, pictureType, "image",
                        Units.toEMU(MAX_IMAGE_WIDTH_PX), Units.toEMU(MAX_IMAGE_HEIGHT_PX));
                }
            } catch (Exception fallbackException) {
                log.warn("Failed to add image with fallback dimensions: {}", fallbackException.getMessage());
            }
        } catch (Exception e) {
